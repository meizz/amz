...
amz._extend(amz.base.Class.prototype, {
     ...
     * @function 
     * @grammar instance.dispose()
     * TODO: 将_listeners中绑定的事件剔除掉
     */
    ,dispose: function() {
        if (this.fire("ondispose")) {
            // decontrol
            delete amz._global_._instances_[this.guid];
    ...
    /**
     * @description 派发自定义事件，使得绑定到自定义事件上面的函数都会被执行。引入amz.base.Event后，Class的子类实例才会获得该方法。
     * @grammar instance.fire(event[, options])
     * @param {amz.base.Event|String}   event   Event对象，或事件名称(1.1.1起支持)
 ...
 * @class
 * @name    amz.base.Event
 * @grammar new amz.base.Event(type[, target])
 * @param   {string} type    事件类型名称。为了方便区分事件和一个普通的方法，事件类型名称必须以"on"(小写)开头。
 * @param   {Object} [target]触发事件的对象
 */
amz.base.Event = function(type, target) {
    this.type = type;
...
//  2011.11.23  meizz   添加 amzInstance 这个全局方法，可以快速地通过guid得到实例对象
